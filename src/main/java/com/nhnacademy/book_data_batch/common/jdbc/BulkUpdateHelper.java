...
/**
 * JDBC Bulk Update를 위한 공통 유틸리티 클래스
 * - 배치 분할, 재시도 로직, PreparedStatement 설정 기능 제공
 * - BulkInsertHelper와 동일한 패턴으로 Update 작업 지원
 */
@Slf4j
@Component
...
public class BulkUpdateHelper {
    ...
    /**
     * Bulk Update 실행 (기본 배치 사이즈 사용)
     *
     * @param sql    UPDATE SQL
     * @param items  업데이트할 아이템들
    ...
    public <T> void bulkUpdate(
        ...
        bulkUpdate(sql, items, setter, DEFAULT_BATCH_SIZE);
    }

    /**
     * Bulk Update 실행 (커스텀 배치 사이즈)
     *
     * @param sql       UPDATE SQL
     * @param items     업데이트할 아이템들
     * @param setter    PreparedStatement에 값을 설정하는 함수
        ...
        while (true) {
            try {
                ...
                if (attempt >= MAX_RETRIES) {
                    log.error("Bulk Update 실패 - {}회 재시도 후 포기", MAX_RETRIES);
                    throw e;
                }
